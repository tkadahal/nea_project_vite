<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200 dark:border-gray-700 col-span-12 lg:col-span-4"
                    ...
                    @if (!empty($availableDirectorates))
                        ...
                        @foreach ($availableDirectorates as $id => $name)
                            <button wire:click="$set('directorateFilter', {{ $id }})"
                                class="directorate-option-project block w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-md {{ $loop->index >= 3 ? 'hidden' : '' }}"
                                data-filter="{{ $id }}"
                                data-name="{{ $name }}">{{ $name }}</button>
                        @endforeach
                        <div id="project-no-results" class="hidden px-3 py-2 text-sm text-gray-500 dark:text-gray-400">
        ...
        <div class="relative w-full max-w-[140px] sm:max-w-[160px] aspect-square">
            <div class="absolute inset-0 flex items-center justify-center">
                <div class="text-center">
                    <p class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white" id="completed-percentage">
    ...
    <div class="flex justify-around text-xs sm:text-sm text-gray-600 dark:text-gray-400">
        <div class="text-center">
            ...
            <span>Completed</span>
            <p class="text-gray-800 dark:text-white font-medium" id="completed-text">{{ $project_status['completed'] }}%
            </p>
        </div>
        <div class="text-center">
        ...
        <div class="text-center">
            ...
            <span>Behind</span>
            <p class="text-gray-800 dark:text-white font-medium" id="behind-text">{{ $project_status['behind'] }}%</p>
        </div>
    </div>
...
<script>
    ...
    waitForjQuery(function($) {
        ...
        $projectContainer.find('.dropdown-toggle-project').on('click', function(e) {
            ...
            $projectContainer.find('.directorate-option-project').each(function(index) {
        ...
        $projectContainer.find('.directorate-option-project').on('click', function() {
            ...
            console.log('Project filter clicked:', filterValue);
        });
