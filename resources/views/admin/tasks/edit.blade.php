<x-layouts.app>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Update Task') }}</p>
        </div>
    ...
    <div
        ...
        <form class="w-full" action="{{ route('admin.task.update', $task) }}" method="POST">
            @csrf
            @method('PUT')
                            ...
                            <div class="col-span-full">
                                <x-forms.select label="Directorate" name="directorate_id" id="directorate_id"
                                    :options="collect($directorates)
                                        ...
                                        ->values()
                                        ->all()" :selected="old('directorate_id', (string) $task->directorate_id)" placeholder="Select directorate"
                            ...
                            <div class="col-span-full">
                                <x-forms.multi-select label="Projects" name="projects[]" id="projects"
                                    :options="collect($projects)
                                        ...
                                        ->all()" :selected="old(
                                        'projects',
                                        $task->projects->pluck('id')->map(fn($id) => (string) $id)->toArray(),
                                    )" placeholder="Select projects" :error="$errors->first('projects')"
                                    class="js-multi-select" />
                            </div>

                            <div class="col-span-full">
                                <x-forms.multi-select label="Assignees" name="users[]" id="users" :options="collect($users)
                                    ->map(fn($label, $value) => ['value' => (string) $value, 'label' => $label])
                                    ->values()
                                    ->all()"
                                    :selected="old(
                                        'users',
                                        $task->users->pluck('id')->map(fn($id) => (string) $id)->toArray(),
                                    )" placeholder="Select assignees" :error="$errors->first('users')"
                                    class="js-multi-select" />
                            </div>

                            <div class="col-span-full">
                                <x-forms.input label="Title" name="title" type="text" :value="old('title', $task->title)"
                                    placeholder="Enter task title" :error="$errors->first('title')" />
                            </div>

                            <div class="col-span-full">
                                <x-forms.text-area label="Description" name="description" :value="old('description', $task->description)"
                ...
                <div class="space-y-6">
                    <div class="p-6 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                <x-forms.date-input label="Due Date" name="due_date" :value="old('due_date', $task->due_date ? $task->due_date->format('Y-m-d') : '')"
                                    :error="$errors->first('due_date')" />
                            </div>
                            <div>
                                <x-forms.date-input label="Completion Date" name="completion_date" :value="old(
                                    'completion_date',
                                    $task->completion_date ? $task->completion_date->format('Y-m-d') : '',
                                )"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-forms.select label="Status" name="status_id" id="status_id" :options="collect($statuses)
                                    ...
                                    ->values()
                                    ->all()"
                                    :selected="old('status_id', (string) $task->status_id)" placeholder="Select status" :error="$errors->first('status_id')"
                            ...
                            <div>
                                <x-forms.select label="Priority" name="priority_id" id="priority_id" :options="collect($priorities)
                                    ...
                                    ->values()
                                    ->all()"
                                    :selected="old('priority_id', (string) $task->priority_id)" placeholder="Select priority" :error="$errors->first('priority_id')"
                                    class="js-single-select" />
                            </div>
                        </div>
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            waitForJQuery(function() {
                ...
                // Initialize multi-select dropdowns with existing data
                const $projectsContainer = $('.js-multi-select[data-name="projects"]');
                const $usersContainer = $('.js-multi-select[data-name="users"]');

                const initialProjects = @json(collect($projects)->map(fn($label, $value) => ['value' => (string) $value, 'label' => $label])->values()->all());
                const initialUsers = @json(collect($users)->map(fn($label, $value) => ['value' => (string) $value, 'label' => $label])->values()->all());
                const taskProjects = @json($task->projects->pluck('id')->map(fn($id) => (string) $id)->toArray());
                const taskUsers = @json($task->users->pluck('id')->map(fn($id) => (string) $id)->toArray());

                console.log('Initial projects:', initialProjects, 'Selected:', taskProjects);
                console.log('Initial users:', initialUsers, 'Selected:', taskUsers);

                $projectsContainer
                    .data('options', initialProjects)
                    .data('selected', taskProjects)
                    .trigger('options-updated', {
                        options: initialProjects,
                        selected: taskProjects
                    });

                $usersContainer
                    .data('options', initialUsers)
                    .data('selected', taskUsers)
                    .trigger('options-updated', {
                ...
                directorateInput.on("change", function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            updateSelectOptions($projectsContainer, formattedData, validProjects);
                            updateSelectOptions($usersContainer, [],
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            const validOldUsers = @json(old('users', []))
                                ...
                                @json(old('users', [])) :
                                taskUsers;
                            const validUsers = validOldUsers.filter((userId) =>
                                formattedData.some((opt) => String(opt.value) === String(
                                    userId))
                            );
                            updateSelectOptions($usersContainer, formattedData, validUsers);
                            if (formattedData.length === 0) {
                                $("#error-message").removeClass("hidden").find("#error-text").text(
                ...
                if (initialDirectorateId && !isNaN(initialDirectorateId) && initialDirectorateId > 0) {
                    ...
                    console.log("No valid initial directorate_id, using initial projects and users");
                    // Trigger users fetch if projects are pre-selected
                    if (taskProjects.length > 0) {
                        debouncedFetchUsers(taskProjects);
                    }
                }
            });
        </script>
