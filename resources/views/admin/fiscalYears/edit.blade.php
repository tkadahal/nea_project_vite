    ...
    <div class="flex flex-col md:flex-row gap-6">
        <div class="flex-1">
            <div
                ...
                <form class="max-w-3xl mx-auto" action="{{ route('admin.fiscalYear.update', $fiscalYear->id) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-full">
                                <x-forms.input label="{{ trans('global.fiscalYear.fields.title') }}" name="title"
                                    type="text" :value="old('title', $fiscalYear->title)" :error="$errors->first('title')" />
                            </div>
                            <div class="col-span-full">
                                <x-forms.date-input label="{{ trans('global.fiscalYear.fields.start_date') }}"
                                    name="start_date" :value="old(
                                        ...
                                        $fiscalYear->start_date
                                            ? \Carbon\Carbon::parse($fiscalYear->start_date)->format('Y-m-d')
                                            : '',
                            ...
                            <div class="col-span-full">
                                <x-forms.date-input label="{{ trans('global.fiscalYear.fields.end_date') }}"
                                    name="end_date" :value="old(
                                        'end_date',
                                        $fiscalYear->end_date
                                            ? \Carbon\Carbon::parse($fiscalYear->end_date)->format('Y-m-d')
                                            : '',
