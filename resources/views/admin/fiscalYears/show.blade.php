<x-layouts.app>
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">
                {{ trans('global.fiscalYear.title_singular') }} {{ trans('global.details') }}
            </h1>
    ...
    <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                    {{ trans('global.fiscalYear.fields.start_date') }} :
                </p>
                <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">
            ...
            <div>
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                    {{ trans('global.fiscalYear.fields.end_date') }} :
                </p>
            ...
            <div>
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                    {{ trans('global.fiscalYear.fields.updated_at') }} :
                </p>
                <p class="mt-1 text-lg text-gray-900 dark:text-gray-100">
                    {{ $fiscalYear->updated_at->format('M d, Y H:i A') }}
                </p>
            </div>
        </div>
        ...
        <div class="mt-6 flex space-x-3">
            ...
            @can('fiscalYear_edit')
                <a href="{{ route('admin.fiscalYear.edit', $fiscalYear) }}"
                    ...
                    {{ trans('global.edit') }} {{ trans('global.fiscalYear.title_singular') }}
                </a>
            @endcan

            @can('fiscalYear_delete')
                <form action="{{ route('admin.fiscalYear.destroy', $fiscalYear) }}" method="POST"
                    onsubmit="return confirm('Are you sure you want to delete this fiscalYear? This action cannot be undone.');">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                        ...
                        {{ trans('global.delete') }} {{ trans('global.fiscalYear.title_singular') }}
                    </button>
                </form>
            @endcan
