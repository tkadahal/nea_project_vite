<x-layouts.app>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Role') }}</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Update Role') }}</p>
        </div>
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-full">
                                <x-forms.input label="Title" name="title" type="text" :value="old('title', $role->title)"
                                    :error="$errors->first('title')" />
                            </div>

                            <div class="col-span-full">
                                <x-forms.multi-select label="Permissions" name="permissions[]" :options="collect($permissions)
                                    ...
                                    )" multiple placeholder="Select permissions" :error="$errors->first('permissions')" />
                            </div>
                        </div>
                    </div>
                    {{-- End Role Information Section --}}
