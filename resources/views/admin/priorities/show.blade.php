<x-layouts.app>
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Priority Details</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Details for: <span
                    class="font-semibold">{{ $priority->title }}</span></p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $priority->title }}</p>
            </div>
            ...
            <div>
                ...
                <p class="mt-1 text-lg text-gray-900 dark:text-gray-100">
                    {{ $priority->created_at->format('M d, Y H:i A') }}</p>
            </div>
            ...
            <div>
                ...
                <p class="mt-1 text-lg text-gray-900 dark:text-gray-100">
                    {{ $priority->updated_at->format('M d, Y H:i A') }}</p>
            </div>
        </div>

        <div class="mt-6 flex space-x-3">
            <a href="{{ route('admin.priority.edit', $priority) }}"
            ...
            <form action="{{ route('admin.priority.destroy', $priority) }}" method="POST"
                onsubmit="return confirm('Are you sure you want to delete this priority? This action cannot be undone.');">
                @csrf
                @method('DELETE')
                <button type="submit"
